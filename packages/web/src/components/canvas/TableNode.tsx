...
function typeColorClass(typeName: string): string {
  ...
  if (/(int|float|double|decimal|numeric|real|serial|number|money|bit)/.test(n)) {
...
function AttrPill({
  ...
  return (
    <span
      className={`rounded-md px-2 py-1 text-[11px] font-medium ${pillClass[tone]}`}
    >
      {label}
    </span>
  );
        ...
        <div className="flex flex-col" onMouseLeave={() => setHovered(null)}>
          {entity.fields.map((field, index) => {
            ...
            return (
              <div
                key={field.id}
                className="box-border flex h-6 shrink-0 items-center gap-2 border-b border-border/50 px-2 text-xs leading-none last:border-b-0 hover:bg-muted/60"
      ...
      >
        {hoveredField ? (
          <div className="w-72 rounded-xl border border-border bg-card p-4 text-card-foreground shadow-lg">
            ...
            {(() => {
              const pills: Array<{ label: string; tone: keyof typeof pillClass }> =
                [];
              ...
              return pills.length > 0 ? (
                <div className="mb-3 flex flex-wrap gap-2">
                  {pills.map((pill) => (
                    <AttrPill key={pill.label} label={pill.label} tone={pill.tone} />
                  ))}
                </div>
              ) : null;
            })()}
