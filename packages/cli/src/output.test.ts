...
test("printDetectionSummary includes entity, enum, relation, and index counts", () => {
  ...
  try {
    ...
    const schema = {
      entities: [{ id: createEntityId("users"), name: "users", kind: "table" as const, fields: [] }],
      enums: [{ id: createEnumId("status"), name: "status", values: ["active"] }],
      relations: [],
      indexes: [{
    ...
    printDetectionSummary("/tmp", source, schema, "http://127.0.0.1:4317/", ["Prisma"]);
...
test("printCliError routes unsupported project messages", () => {
  ...
  try {
    printCliError(new Error("No schema source found. Supported formats: Prisma."));
    const output = errors.join("\n");
