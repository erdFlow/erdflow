...
pnpm --filter @erdflow/prisma build    # rebuild after parser-prisma changes
# faster CLI-only rebuild (skips web):
# cd packages/prisma && pnpm exec tsup && cp -R public dist/public
```
...
# v6-style
node packages/prisma/dist/cli.js --no-open --prisma packages/parser-prisma/fixtures/v6-classic/schema.prisma

# v7 url-less datasource
node packages/prisma/dist/cli.js --no-open --prisma packages/parser-prisma/fixtures/v7-url-less/schema.prisma

# v7 multi-file folder
node packages/prisma/dist/cli.js --no-open --prisma packages/parser-prisma/fixtures/v7-multifile/prisma/schema

# v7 custom path via config (run from that fixture root for auto-detect)
cd packages/parser-prisma/fixtures/v7-config-custom
node ../../../prisma/dist/cli.js --no-open
# or from monorepo root:
# node packages/prisma/dist/cli.js --no-open --prisma packages/parser-prisma/fixtures/v7-config-custom/db/schema.prisma
```
...
```bash
# Single schema file
node packages/prisma/dist/cli.js --no-open --prisma /absolute/path/to/schema.prisma

# Multi-file schema directory
node packages/prisma/dist/cli.js --no-open --prisma /absolute/path/to/prisma/schema

# Auto-detect from project root (schema.prisma, prisma/schema/, or prisma.config.ts)
cd /path/to/your/prisma-project
node /absolute/path/to/erdFlow/packages/prisma/dist/cli.js --no-open
```
...
node packages/prisma/dist/cli.js --prisma packages/parser-prisma/fixtures/basic.prisma
```
