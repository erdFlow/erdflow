...
| Parse error on url-less v7 schema | Rebuild the CLI so `dist` includes the stub: `cd packages/cli && pnpm exec tsup` (or `pnpm --filter erdflow build`). Source already injects a DMMF-only stub URL; stale `dist/cli.js` will still fail. |
| Schema save does not update UI | CLI shows `Watching…`? Save a watched file; check CLI for parse errors; footer **Connected**? |
| `pnpm erdflow` arg errors | Prefer `node packages/cli/dist/cli.js --no-open --prisma …` from monorepo root |
